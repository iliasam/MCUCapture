...
/* Set data value watchpoint for COMP1 */
int cortex_m_set_value_watchpoint(
	...
	uint32_t mask)
{
	...
	/* Always COMP1 */
	comparator = &cortex_m->dwt_comparator_list[1];
	comparator->used = true;
	
	/* We do not need to change watchpoint->set value here*/
	
	comparator->comp = watchpoint->address;
	
	/* Set data value */
	target_write_u32(target,
		comparator->dwt_comparator_address + 0,
		watchpoint->value);
	
	comparator->mask = mask;
	target_write_u32(target,
		comparator->dwt_comparator_address + 4,
		comparator->mask);
	...
	/* Set DATAVMATCH bit*/
	comparator->function |= (1 << 8);
	
	target_write_u32(target,
		comparator->dwt_comparator_address + 8,
		comparator->function);
	
	LOG_DEBUG("Value Watchpoint (ID %d) DWT%d 0x%08x 0x%x 0x%05x",
...
int cortex_m_set_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	...
	if ((watchpoint->value != 0) && (dwt_num != 0)) {
		LOG_ERROR("Can not set value watchpoint: COMP0 is already used");
		return ERROR_FAIL;
	}
	/* If value data mode is used, COMP0 is used for address comparasion*/
	...
	if (watchpoint->value != 0) {
		cortex_m_set_value_watchpoint(target, watchpoint, mask);
	}
	
	return ERROR_OK;
}

/* Unset watchpoint with setted data value*/
int cortex_m_unset_value_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	struct cortex_m_common *cortex_m = target_to_cm(target);
	struct cortex_m_dwt_comparator *comparator;
	...
	LOG_DEBUG("Value Watchpoint (ID %d) DWT%d address: 0x%08x clear",
		watchpoint->unique_id,
		1,
...
int cortex_m_unset_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	...
	if (watchpoint->value != 0) {
		cortex_m_unset_value_watchpoint(target, watchpoint);
	}
...
int cortex_m_add_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	...
	if (watchpoint->value) {
		if ((cortex_m->dwt_devarch & 0x1FFFFF) == DWT_DEVARCH_ARMV8M) {
			LOG_DEBUG("data value watchpoint for ARMV8M not YET supported");
			return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
		} else {
			if (cortex_m->dwt_comp_available == cortex_m->dwt_num_comp) {
				/* Current realization needs all comparators to be free */
				/* We need two comparators: COMP0 and COMP1 */
				cortex_m->dwt_comp_available--;
			} else {
				LOG_DEBUG("All comparators must be free for setting value watchpoint");
				return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
			}
		}
	}
